...
for c1 in range(97, 123):
    for c2 in range(97, 123):
        for c3 in range(97, 123):
            decipher = []
            for x in range(len(cipher) // 3):
                decipher.append(chr(c1 ^ cipher[3 * x]))
                decipher.append(chr(c2 ^ cipher[3 * x + 1]))
                decipher.append(chr(c3 ^ cipher[3 * x + 2]))
            cipher_dict[(chr(c1),chr(c2),chr(c3))] = ''.join(decipher)

for x in cipher_dict:
    if cipher_dict[x].find(' the ') != -1:
        cipher_string = cipher_dict[x][:-2]
result = [ord(char) for char in cipher_string]
print(sum(result))