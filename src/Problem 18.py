...
for x in range(len(tri_dict.keys())):
    tri_dict[x] = [int(stuff) for stuff in tri_dict[x]]

size = len(tri_dict.keys())
for x in range(1, size):
    tri_dict[size - x - 1] = [elem + max(tri_dict[size - x][i:i+2]) for i, elem in enumerate(tri_dict[size - x - 1])]
